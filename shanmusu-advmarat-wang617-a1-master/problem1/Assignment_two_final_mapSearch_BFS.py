...
def search_algorithm():
    ...
    while not end_program:
        #print ("opened_list" , opened_list[len(opened_list)-1].getcity_name());
        ...
        if len(closed_list) > 0:
            if closed_list[0].getcity_name() == goal_node:
                end_program = True;
                goal_reached = True;
                opened_list.append(closed_list[0]);
                closed_list.remove(closed_list[0]);
            else:
                opened_list.append(closed_list[0]);
                closed_list.remove(closed_list[0]);
        else:
