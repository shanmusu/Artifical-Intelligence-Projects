...
def getSuccessors(current_city_object):
    ...
    for highway_object_list_item in highway_object_list:
        ...
        if highway_object_list_item.getstart_city() == current_city_object.getcity_name():
            ...
            city.my_level = current_city_object.my_level + 1
...
def search_algorithm():
    ...
    while not goal_reached:
        end_program = False
        opened_list = []
        opened_list.append(City(start_node,0,0,0))
        count+= 1
        #print count
        while not end_program:
            if opened_list[len(opened_list)-1].my_level <= count:
                successors = getSuccessors(opened_list[len(opened_list)-1]);
                #print opened_list[len(opened_list)-1].getcity_name()
                #print opened_list[len(opened_list)-1].my_level
            else:
                successors = []
